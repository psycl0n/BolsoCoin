...
#if defined(NDEBUG)
# error "Bolsocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x01ba743ece856516f61b43b12c4361025a35a8fed3d7cf6dee5ea392a7106ab0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bolsocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bolsocoin Signed Message:\n";
...
{
    // Bolsocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 1488 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // Bolsocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Bolsocoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Bolsocoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bolsocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bolsocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "O futuro da pedofilia";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1509211682;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087355558;
        ...
        assert(block.hashMerkleRoot == uint256("0x6c43adb626185179a70873687e81ef25f98ae89a1362d9f9cfb2ac39e625d14a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bolsocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BolsocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BolsocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BolsocoinMiner(CWallet *pwallet)
{
    printf("BolsocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Bolsocoin-miner");
    ...
    try { loop {
        ...
        printf("Running BolsocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BolsocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BolsocoinMiner, pwallet));
}
